...
def ad_title(title: str) -> str:
    """
    제목 앞에 [광고] 를 붙인다. 이미 있으면 그대로 둔다.

    검색결과에서 제목은 40자 안팎에서 잘리므로 표시가 4자를 먹는다.
    그래도 빼면 안 된다 — 표시 누락은 승인 거부 사유다.
    """
    t = (title or "").strip()
    if not t:
        return AD_MARK
    if re.match(r"^\s*\[?\s*(광고|AD|Ad|ad)\s*\]?", t):
        return t
    return f"{AD_MARK}{t}"
...
_CATEGORY_RULES = [
    ...
    (("뷰티", "화장", "스킨", "헤어", "네일"), "뷰티"),
    (("생수", "음료", "커피", "과자", "냉동", "정육", "농산"), "식품"),
    (("욕실", "청소", "세제", "위생", "생활"), "생활용품"),
    (("스포츠", "레저", "캠핑", "등산", "자전거", "낚시"), "스포츠/레저"),
]
...
def map_blog_category(product: dict) -> str:
    ...
    return DEFAULT_BLOG_CATEGORY                    # 3) 부모 카테고리로 보낸다

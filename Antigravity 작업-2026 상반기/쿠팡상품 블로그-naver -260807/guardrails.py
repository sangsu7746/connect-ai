...
#: 글 쓰는 모델에게 미리 알려 줄 금지 표현. 위 CAUSAL·SUPERLATIVE·FIRST_PERSON 과
#: 같은 것을 사람 말로 옮긴 것이므로 **위 목록을 고치면 여기도 같이 고친다.**
#:
#: 왜 굳이 두 번 쓰는가: 검사만 두면 모델이 모르고 쓰고, 쓰면 그 글은 통째로 버려진다.
#: 밤중에 10건 도는 작업에서 서너 건이 그렇게 날아갔다. 검사는 그대로 두되(믿을 건 검사다),
#: 애초에 안 쓰게 미리 일러두는 편이 헛품을 던다.
PROMPT_BAN_TEXT = """[절대 쓰면 안 되는 표현 — 하나라도 나오면 글 전체가 폐기된다]
· 확인할 수 없는 인과: '덕분에', '~때문에 가능한 가격', '원가를 낮춰서', '유통 단계를 줄여서'
· 근거 없는 단정: '증명합니다', '입증된', '검증된', '보장합니다', '신선한'
· 상품평 수를 만족·품질로 바꿔 말하기: '만족도가 높습니다', '재구매율', '반품률', '불량률'
· 최상급과 순위: '최고의', '최적의 선택', '최저가', '가장 싼', '1위', 'NO.1', '국내 최초', '유일한'
· 조급함 부추기기: '지금이 기회', '놓치면', '서두르세요', '품절 임박', '한정 수량', '마감 임박'
· 강권: '강력 추천', '무조건 사세요', '안 사면 후회', '가성비 갑'
· 써 보지 않은 것을 써 본 척: '직접 써보니', '제가 사용해보니', '내돈내산', '솔직 후기', '사용기'

[숫자 규칙]
주어진 [실측 데이터]에 있는 숫자만 쓴다. 거기에 없는 숫자는 단 하나도 지어내지 마라.
개당 단가처럼 주어진 값에서 나눗셈·뺄셈으로 바로 나오는 값은 계산해 써도 된다.
'구매자의 80%', '10명 중 9명' 같은 값은 데이터에 없으므로 쓰면 폐기된다."""

...
def posts_today(channel_id=None, exclude_id=None, platform=None,
                account=None) -> int:
    ...
    account: 그 계정만 센다.
      ⚠ 상한의 목적은 **계정 잠김 방지**다. 위험은 계정마다 따로 쌓이므로
        세는 것도 계정마다 따로여야 한다. platform 만으로 세면 같은 플랫폼의
        두 번째 계정이 첫 번째 계정의 발행 때문에 막힌다 — 한쪽이 조용한데도
        다른 쪽이 상한에 걸리는, 보호가 아니라 손해다.
        (밴드 계정이 하나뿐이던 동안에는 드러나지 않던 결함이다.)
        빈 문자열·None 은 '계정 구분 없음'이라 필터를 걸지 않는다."""
    ...
    if account:
        q += " AND channel_id IN (SELECT id FROM channels WHERE account=?)"
        args.append(account)
    with get_conn() as c:
        return int(c.execute(q, args).fetchone()[0])


def last_post_time(platform=None, account=None):
    ...
    account: 그 계정의 마지막 발행만 본다.
      ⚠ 같은 이유로 계정도 나눠야 한다. 계정 A 가 방금 올렸다고 계정 B 를
        기다리게 할 이유가 없다 — 밖에서 보면 서로 다른 사람이다."""
...
def channels_for_profile(profile_key: str, enabled_only: bool = True) -> list:
    ...
    ⚠ 순서가 중요하다 — tools/run_by_profile.py 가 이 목록을 앞에서부터
      chans[:limit] 으로 자른다. 순서 없이 반환하면 SQLite 기본 순서(=대개
      rowid, 즉 채널 id 순)를 따르는데, 그러면 **id 가 낮은 채널이 매번
      이긴다.** 주기당 예산을 여러 업종이 나눠 가지면 limit 이 작아지고,
      id 가 높은 채널은 그 업종이 존재하는 한 영원히 뽑히지 않는다.
      실측(2026-08-17): headjim100 계정의 밴드 채널 47곳이 2026-08-12 이후
      단 한 번도 새 소재를 못 받았다 — 같은 업종을 공유하는 headjimkss
      채널들의 id 가 더 낮아 매 주기 그쪽만 chans[:limit] 안에 들었기 때문.

    ⚠ '가장 오래 발행 안 된 채널을 앞에 둔다'(=NULL 우선)로 짜 봤지만
      이 규모에서는 그게 오히려 새로운 고착을 만든다. 실측(2026-08-17,
      mirizip 26곳): 23곳이 '한 번도 안 나감(NULL)', 3곳만 발행 이력이
      있었다(그중 ch470·471 은 2026-08-12, 닷새 전). NULL 을 최우선으로
      두면 그 23곳이 남아 있는 한 나머지 3곳은 **영원히** 밀린다 —
      채널이 계속 늘어나는 이 프로젝트에서 '한 번도 안 나간 채널' 풀은
      절대 마르지 않으므로, 한 번이라도 나갔던 채널은 그 순간부터
      다시는 못 뽑히는 구조가 된다(정확히 headjim100 47곳이 겪은 문제).

    그래서 이력을 아예 안 본다. 순수 무작위로 섞는다 — 주기가 하루에도
    수십 번 도니, 여러 주기에 걸쳐 보면(누적) 계정·채널 상관없이
    고르게 뽑힌다. 한 번의 호출로 공평해지는 설계가 아니라 **오래
    지켜보면** 공평해지는 설계다.
    """
...
def is_demo_channel(row) -> bool:
    """존재하지 않는 시연용·시험용 채널인가. 켜두면 발행이 늘 실패하는데
    원인이 '가짜 주소'라는 걸 알아채기 어렵다.

    ⚠ 더 나쁜 경우도 있다. 'band.us/band/1' 같은 자리표시자 주소는
      **실재하는 남의 밴드**일 수 있다. 그대로 발행되면 모르는 사람 밴드에
      광고가 올라간다(실측: '테스트밴드'가 승인 대기열에 들어와 있었고,
      예전 규칙으로는 걸러지지 않았다)."""
    if not row:
        return False
    d = dict(row)
    name = str(d.get("name", ""))
    ref = str(d.get("target_ref", ""))
    blob = f"{name} {ref}".upper()
    if any(k in blob for k in ("DEMO", "데모", "TEST", "테스트", "샘플", "SAMPLE", "시험")):
        return True
    # 밴드/그룹 번호가 비정상적으로 작으면 자리표시자다(실제 번호는 8~9자리).
    m = re.search(r"/(?:band|groups)/(\d+)/?$", ref)
    return bool(m) and len(m.group(1)) < 5
...
# ── 폴백 문구 차단 ──────────────────────────────────────────
# 카피 생성이 실패하면 orchestrator._fallback_caption 이 업종마다 **고정된**
# 문장을 넣는다. 그 자체는 안전장치지만, 발행까지 그대로 흘러가면 같은 방에
# 몇 번을 올려도 글이 똑같아진다.
#
# ⚠ 2026-08-13 실제 사고: 구글 프로젝트가 미결제로 막히자(403) 문구 생성이
#   전량 폴백으로 떨어졌는데 아무도 몰랐다. 예외 메시지를 로그가 버리고 있어
#   조용했다. 그 사이 ch471·ch472 등에 **똑같은 글이 6번씩** 나갔다.
#   '다른 문구로 올리니 하루 여러 번 괜찮다'는 운영 전제가 그때 무너졌다.
#
# ⚠ 그래서 생성이 아니라 **발행**에서 막는다. 생성을 막으면 API 가 살아난 뒤
#   다시 만들어야 하지만, 발행만 막으면 폴백은 큐에 남아 있다가 그냥 무시되고
#   결제가 풀리는 즉시 정상 문구부터 나간다. 사람이 개입할 필요가 없다.
#
# 끄려면 .env 에 BLOCK_FALLBACK_PUBLISH=0. 기본은 켬 — 막는 쪽이 안전하다.
def block_fallback_publish() -> bool:
    return os.getenv("BLOCK_FALLBACK_PUBLISH", "1").strip() != "0"


def not_fallback_sql(alias: str = "cr") -> str:
    """발행 대상 질의에 붙일 조건. 켜져 있지 않으면 항상 참인 조건을 준다.

    ⚠ '_fallback' 키는 폴백일 때만, 값 True 로만 기록된다
      (orchestrator._fallback_caption). 그래서 **키 존재 여부**로 판정하면
      되고, JSON 공백 차이에 흔들리지 않는다.
    """
    if not block_fallback_publish():
        return "1=1"
    return f"COALESCE({alias}.copy_json,'') NOT LIKE '%\"_fallback\"%'"


def is_fallback_copy(copy_json) -> bool:
    """파이썬 쪽에서 같은 판정이 필요할 때. SQL 조건과 뜻이 같아야 한다."""
    return '"_fallback"' in (copy_json or "")

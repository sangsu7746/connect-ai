...
def test_reply_with_supplementary_plane_emoji_uses_cdp_not_send_keys(pub, no_sleep):
    """실측(2026-08-05): 답글에 🏡(U+1F3E1, BMP 밖) 이 있으면
    box.send_keys(ch) 가 "ChromeDriver only supports characters in the
    BMP" 로 실패했다. reply_writer.validate() 는 이모지를 최대 2개까지
    명시적으로 허용하므로 이건 예외적 입력이 아니라 이 파이프라인이
    정상적으로 만들어내는 답글의 범위다 — 🏠🛠️🥹✨ 등 흔한 이모지
    대부분이 BMP 밖이라 같은 문제를 겪는다.

    BMP 밖 문자만 CDP Input.insertText 로 넣고, 나머지는 기존처럼
    send_keys 로 타이핑해야 한다(인간형 리듬은 유지)."""
    text = "좋아요 🏡 이거 정말 예뻐요"
    box = _FakeElement()
    perm = _FakeElement(href="https://www.threads.net/@tester/post/NEW1")
    driver = _attach_fake(pub, elements={BOX_SEL: [box], PERM_SEL: [perm]})
    r = pub.reply("https://www.threads.net/@a/post/1", text, dry_run=False)
    assert r.ok is True

    # 이모지 자체는 send_keys 호출 목록에 없어야 한다(CDP 로 우회했으므로).
    typed_via_send_keys = "".join(
        c[0] for c in box.send_keys_calls[:-1] if c and isinstance(c[0], str))
    assert "🏡" not in typed_via_send_keys

    # CDP 로는 그 한 글자만 정확히 들어가야 한다.
    assert driver.cdp_calls == [("Input.insertText", {"text": "🏡"})]

    # 두 경로를 합치면 원문과 정확히 같아야 한다(순서 보존).
    rebuilt = []
    cdp_iter = iter(c[1]["text"] for c in driver.cdp_calls)
    for ch in text:
        if ord(ch) > 0xFFFF:
            rebuilt.append(next(cdp_iter))
        else:
            rebuilt.append(ch)
    assert "".join(rebuilt) == text


def test_reply_without_emoji_never_calls_cdp(pub, no_sleep):
    """BMP 안 문자만 있는 흔한 경우 CDP 경로를 아예 타지 않아야 한다 —
    불필요한 CDP 호출은 그 자체로 로그·타이밍이 달라져 탐지 신호가 될
    수 있다."""
    box = _FakeElement()
    driver = _attach_fake(pub, elements={BOX_SEL: [box], PERM_SEL: []})
    r = pub.reply("https://www.threads.net/@a/post/1", "이모지 없는 평범한 답글", dry_run=False)
    assert r.ok is True

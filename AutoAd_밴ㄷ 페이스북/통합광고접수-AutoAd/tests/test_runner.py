...
# 이 파일의 테스트는 '쓰레드 자동발행 라우팅'을 검증한다. 그러려면 쓰레드로
# 나가도 되는 업종이어야 한다 - profile 에 key 를 안 주면 runner 가
# config.PROFILE_KEY(=loan) 로 떨어지고, loan 은 allow_platforms 가
# [band, facebook] 이라 업종 게이트가 자동발행을 전부 승인 큐로 돌린다
# (그것이 이 게이트의 목적이다). 그래서 여기서는 쓰레드 허용 업종을 명시한다.
THREADS_TEST_PROFILE = "mirizip"


def test_routes_by_score(wired):
    res = wired.run_once(account="tester", profile={"key": THREADS_TEST_PROFILE, "threads": {
...
def test_empty_harvest_is_not_an_error(wired, monkeypatch):
    ...
    res = wired.run_once(account="tester", profile={"key": THREADS_TEST_PROFILE, "threads": {}}, dry_run=True)
...
def test_cooldown_checked_before_reply_generation(wired, monkeypatch):
    ...
    monkeypatch.setattr("db.threads_author_replied_since",
                        lambda author, days: author == "@u0")
    wired.run_once(account="tester", profile={"key": THREADS_TEST_PROFILE, "threads": {
        "interest_keywords": ["보정"], "hard_block": [], "landing": "https://x.test"}},
        dry_run=True)
...
def test_run_once_is_idempotent_across_passes(wired, monkeypatch):
    ...
    kwargs = dict(account="tester", profile={"key": THREADS_TEST_PROFILE, "threads": {
        "interest_keywords": ["보정"], "hard_block": [], "landing": "https://x.test"}},
        dry_run=True)
...
def test_pending_approval_from_earlier_pass_blocks_same_author(wired, monkeypatch):
    ...
    res1 = wired.run_once(account="tester", profile={"key": THREADS_TEST_PROFILE, "threads": {
        "interest_keywords": ["보정"], "hard_block": [], "landing": "https://x.test"}},
        dry_run=True)
    ...
    res2 = wired.run_once(account="tester", profile={"key": THREADS_TEST_PROFILE, "threads": {
        "interest_keywords": ["보정"], "hard_block": [], "landing": "https://x.test"}},
        dry_run=True)
...
def test_loan_profile_never_auto_publishes_to_threads(wired):
    """★ 안전 성질 - 대출 업종은 고득점 답글이라도 쓰레드로 자동발행되지
    않는다. 이 경로는 publish_creative() 를 안 거치므로 게이트가 없으면
    '점수가 낮으면 막히고 높으면 나가는' 뒤집힌 상태가 된다.
    THREADS_PROFILE 이 .env 에서 빠지면 실제로 이 상태가 된다."""
    res = wired.run_once(account="tester", profile={"key": "loan", "threads": {
        "interest_keywords": ["보정"], "hard_block": [], "landing": "https://x.test"}},
        dry_run=True)
    assert res["auto_published"] == 0, "대출 답글이 쓰레드로 자동발행됐다"
    assert res["queued"] >= 1, "차단된 건이 승인 큐로도 안 갔다(기회를 버렸다)"
